...
def length(csv):
    with open(csv) as csv_file:
        return sum(1 for i in csv_file)
...
def _align_reads():
    ...
    with open(CSV_FILE) as csv_file:
        ...
        output_file_count = 1
        for row in ChargingBar('Processing', max=length(CSV_FILE)).iter(csv_reader):
...
def _build_indexes():
    output_file = open("batch_out.txt","w") 
    ...
    for ref_seq_file in ref_seq_file_list:
        ...
        subprocess.run(run_args, stdout=output_file)
    output_file.close()
